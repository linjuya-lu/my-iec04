                  ...
                  bool isSequence)
{
    ...
    case FILE210_OP_READ_ACT_ACK: {
        ...
        for (int i = 0; i < self->u.readActAck.nameLen; i++)
            Frame_setNextByte(frame, (uint8_t)self->u.readActAck.name[i]);

        uint32_t id = self->u.readActAck.fileId;
        Frame_setNextByte(frame, (uint8_t)( id        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 24) & 0xff));

        uint32_t fs = self->u.readActAck.fileSize;
        Frame_setNextByte(frame, (uint8_t)( fs        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 16) & 0xff));
    ...
    case FILE210_OP_READ_DATA: {
        ...
        uint32_t seg = self->u.readData.segNo;
        Frame_setNextByte(frame, (uint8_t)( seg        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 24) & 0xff));

        Frame_setNextByte(frame, self->u.readData.hasMore);
        for (int i = 0; i < self->u.readData.dataLen; i++)
            Frame_setNextByte(frame, self->u.readData.data[i]);
    ...
    case FILE210_OP_READ_DATA_ACK: {
        ...
        uint32_t seg = self->u.readDataAck.segNo;
        Frame_setNextByte(frame, (uint8_t)( seg        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 24) & 0xff));
    ...
    case FILE210_OP_WRITE_ACT: {
        ...
        for (int i = 0; i < self->u.writeAct.nameLen; i++)
            Frame_setNextByte(frame, (uint8_t)self->u.writeAct.name[i]);

        uint32_t id = self->u.writeAct.fileId;
        Frame_setNextByte(frame, (uint8_t)( id        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 24) & 0xff));

        uint32_t fs = self->u.writeAct.fileSize;
        Frame_setNextByte(frame, (uint8_t)( fs        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 24) & 0xff));
    ...
    case FILE210_OP_WRITE_ACT_ACK: {
        ...
        Frame_setNextByte(frame, self->u.writeActAck.nameLen);
        for (int i = 0; i < self->u.writeActAck.nameLen; i++)
            Frame_setNextByte(frame, (uint8_t)self->u.writeActAck.name[i]);

        uint32_t id = self->u.writeActAck.fileId;
        Frame_setNextByte(frame, (uint8_t)( id        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((id >> 24) & 0xff));

        uint32_t fs = self->u.writeActAck.fileSize;
        Frame_setNextByte(frame, (uint8_t)( fs        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((fs >> 24) & 0xff));
    ...
    case FILE210_OP_WRITE_DATA: {
        ...
        uint32_t seg = self->u.writeData.segNo;
        Frame_setNextByte(frame, (uint8_t)( seg        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 24) & 0xff));

        Frame_setNextByte(frame, self->u.writeData.hasMore);
        for (int i = 0; i < self->u.writeData.dataLen; i++)
            Frame_setNextByte(frame, self->u.writeData.data[i]);
    ...
    case FILE210_OP_WRITE_DATA_ACK: {
        ...
        uint32_t seg = self->u.writeDataAck.segNo;
        Frame_setNextByte(frame, (uint8_t)( seg        & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 8 ) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 16) & 0xff));
        Frame_setNextByte(frame, (uint8_t)((seg >> 24) & 0xff));
...
static FileExt210
FileExt210_new(int ioa, uint8_t op)
{
                           ...
                           uint8_t       checksum)
{
    ...
    if (data && dataLen > 0) {
        ...
        if (self->u.writeData.data) {
            memcpy((void *)self->u.writeData.data, data, dataLen);
            self->u.writeData.dataLen = dataLen;
        }
        else {
                         ...
                         int startIndex, bool isSequence)
{
    ...
    case FILE210_OP_DIR_CALL:
    {
        ...
        if (tcopy > 0)
            memcpy(self->u.dirCall.beginTime, msg + idx, tcopy);
        if (tcopy < 7)
        ...
        if (tcopy > 0)
            memcpy(self->u.dirCall.endTime, msg + idx, tcopy);
        if (tcopy < 7)
    ...
    {
        if (idx >= msgSize) {
            DEBUG_PRINT("FileExt210 READ_ACT: no nameLen\n");
            GLOBAL_FREEMEM(self);
            return NULL;
        }
    ...
    case FILE210_OP_READ_DATA:
    {
        ...
        if (self->u.readData.dataLen > 0) {
            ...
            if (self->u.readData.data)
                memcpy((void *) self->u.readData.data, msg + idx, self->u.readData.dataLen);
            idx += self->u.readData.dataLen;
        }
        else {
    ...
    {
        if (idx + 9 > msgSize) {
            DEBUG_PRINT("FileExt210 READ_DATA_ACK: too small\n");
            GLOBAL_FREEMEM(self);
            return NULL;
        }
    ...
    case FILE210_OP_WRITE_DATA:
        ...
        if (self->u.writeData.dataLen > 0) {
            ...
            if (self->u.writeData.data)
                memcpy((void*)self->u.writeData.data, msg + idx, self->u.writeData.dataLen);
            idx += self->u.writeData.dataLen;
        }
        else {
            self->u.writeData.data = NULL;
        }
    ...
    case FILE210_OP_WRITE_DATA_ACK:
        ...
        self->u.writeDataAck.fileId = get_le32(msg + idx); idx += 4;
