...
typedef struct
{
    uint8_t  nameLen;                             /* 文件名长度 x */
    char     name[FILE210_MAX_NAME + 1];          /* 文件名（不带 '\0'），多留 1 个字节方便加 '\0' */
    uint8_t  attr;                                /* 文件属性（备用） */
    uint32_t size;                                /* 文件大小（字节数） */
    uint8_t  time[7];                             /* 文件时间，CP56Time2a 原始 7 字节 */
...
struct sFileExt210
{
    ...
    TypeID               type;               /* = M_FT_EXT_1 (210) */
    InformationObjectVFT virtualFunctionTable;

    uint8_t op;          /* FILE210_OP_xxx 之一 */
    ...
    union {
        ...
        /* 对应 7.6.3.1 a) */
        struct {
            uint32_t dirId;                          /* 目录 ID，4 字节，低字节在前 */
            uint8_t  nameLen;                        /* 目录名长度 x（=0 表示读默认目录） */
            char     name[FILE210_MAX_NAME + 1];     /* 目录名（不带结尾 0） */

            uint8_t  callFlag;                       /* 召唤标志：0 全部文件；1 按时间段 */
            uint8_t  beginTime[7];                   /* 查询起始时间 CP56Time2a */
            uint8_t  endTime[7];                     /* 查询终止时间 CP56Time2a */
        } dirCall;
